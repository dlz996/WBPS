...
+ (instancetype)sharedInstance {
    static MMMaterialDesignSpinner *spinnerView = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        spinnerView = [[MMMaterialDesignSpinner alloc] initWithFrame:CGRectMake(0, 0, 60, 60)];
        spinnerView.tintColor = Red_Color;
        spinnerView.hidesWhenStopped = YES;
        spinnerView.lineWidth = 3.0f;
        spinnerView.center = [UIApplication sharedApplication].keyWindow.center;
...
+ (void)startAnimating {
    [[MMMaterialDesignSpinner sharedInstance] startAnimating];
}

+ (void)stopAnimating {
    [[MMMaterialDesignSpinner sharedInstance] stopAnimating];
}
