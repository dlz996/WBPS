...
- (void)viewDidLoad {
    ...
    [self.tableView registerClass:[RecordTableCell class] forCellReuseIdentifier:CellID];
...
- (void)downPullData{
    ...
    upData = [NetWorkingManager combinationObj:@"Query" proc:@"QSP_GET_withdrawals" pars:mutDic];
    
    [[NetWorkingManager sharedManager]POSTWithParameters:upData success:^(id responseObject) {
        [self.dataArray removeAllObjects];
        self.index = 1;
        [self.dataArray addObjectsFromArray:[RecordModel mj_objectArrayWithKeyValuesArray:responseObject[@"msg"]]];
...
- (void)upPullData{
    ...
    [[NetWorkingManager sharedManager]POSTWithParameters:upData success:^(id responseObject) {
        NSArray * array =[RecordModel mj_objectArrayWithKeyValuesArray:responseObject[@"msg"]];
